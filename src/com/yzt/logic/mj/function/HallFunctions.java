...
public class HallFunctions extends TCPGameFunctions {
	...
	public synchronized static void interface_100007(WSClient channel, Map<String, Object> readData) {
		...
		Integer roomType = StringUtils.parseInt(readData.get("roomType"));// 房间类型：1房主模式；2自由模式

		
		Integer scoreType = StringUtils.parseInt(readData.get("scoreType"));// 计分方式：1点炮三家付；2点炮包三家
		Integer ruleJueGang = StringUtils.parseInt(readData.get("ruleJueGang"));// 绝杠选项：1绝；2杠
		Integer ruleXuanFeng = StringUtils.parseInt(readData.get("ruleXuanFeng"));// 是否有旋风杠：1有；2无
		Integer ruleChangMao = StringUtils.parseInt(readData.get("ruleChangMao"));// 是否长毛：1是；2无
		Integer ruleTuiDaoHu = StringUtils.parseInt(readData.get("ruleTuiDaoHu"));// 是否有推倒胡：1有；2无
		Integer ruleDaiHun = StringUtils.parseInt(readData.get("ruleDaiHun"));// 是否带混：1带；2不带
		Integer ruleQiongHu = StringUtils.parseInt(readData.get("ruleQiongHu"));// 是否带穷胡：1是；2无
		Integer ruleQingYiSe = StringUtils.parseInt(readData.get("ruleQingYiSe"));// 是否有清一色：1是；2无
		Integer ruleQiDui = StringUtils.parseInt(readData.get("ruleQiDui"));// 是否有七对：1是；2无
		Integer rulePuFen = StringUtils.parseInt(readData.get("rulePuFen"));// 是否带铺分：1是；2无
		Integer playType = StringUtils.parseInt(readData.get("playType"));// 玩法类型：1经典；2盖州
		...
		//房间规则
		room.setRuleChangMao(ruleChangMao);
		room.setRuleDaiHun(ruleDaiHun);
		room.setRuleJueGang(ruleJueGang);
		room.setRuleQiDui(ruleQiDui);
		room.setRuleQingYiSe(ruleQingYiSe);
		room.setRuleQiongHu(ruleQiongHu);
		room.setRuleTuiDaoHu(ruleTuiDaoHu);
		room.setRuleXuanFeng(ruleXuanFeng);
		room.setRulePuFen(rulePuFen);
...
//		roomInfo.put("playType", room.getPlayType());
	...
	public synchronized static void interface_100008(WSClient channel, Map<String, Object> readData) {
		...
		//规则
		info.put("ruleJueGang", room.getRuleJueGang());
		info.put("ruleXuanFeng", room.getRuleXuanFeng());
		info.put("ruleChangMao", room.getRuleChangMao());
		info.put("ruleTuiDaoHu", room.getRuleTuiDaoHu());
		info.put("ruleDaiHun", room.getRuleDaiHun());
		info.put("ruleQiongHu", room.getRuleQiongHu());
		info.put("ruleQingYiSe", room.getRuleQingYiSe());
		info.put("ruleQiDui", room.getRuleQiDui());
		info.put("rulePuFen", room.getRulePuFen());
		...
		if (hgetAll != null && hgetAll.size() > 0) {
			...
			for (String roomId : keySet) {
				...
				if (room != null && room.getCreateId().equals(userId) && room.getState() != Cnst.ROOM_STATE_YJS && room.getRoomType() == Cnst.ROOM_TYPE_2) {
					...
					//规则
					map.put("ruleJueGang", room.getRuleJueGang());
					map.put("ruleXuanFeng", room.getRuleXuanFeng());
					map.put("ruleChangMao", room.getRuleChangMao());
					map.put("ruleTuiDaoHu", room.getRuleTuiDaoHu());
					map.put("ruleDaiHun", room.getRuleDaiHun());
					map.put("ruleQiongHu", room.getRuleQiongHu());
					map.put("ruleQingYiSe", room.getRuleQingYiSe());
					map.put("ruleQiDui", room.getRuleQiDui());
					map.put("rulePuFen", room.getRulePuFen());
